...
Read the `version:` field from this file's own YAML frontmatter. Every Markdown file this skill writes (`RESEARCH.md`, `INSTALL-PLAN.md`) must begin with a metadata block of the form:

```markdown
<!-- Generated by install-cncf-project v<version-from-frontmatter> on <YYYY-MM-DD> -->
```

CI rewrites the frontmatter `version:` field to the published version before the skill is packaged, so this stamp identifies exactly which skill release produced the file.
